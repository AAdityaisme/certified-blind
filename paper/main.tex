...
\else
You cannot audit what you threw away, not from the data that survives. AI \emph{gatekeepers}---classifiers
deciding which data flows and which is discarded---are increasingly deployed where their decisions are
\textbf{irreversible}: pre-persistence content filters and data-curation pipelines drop inputs before storage;
onboard satellites discard ``cloudy'' scenes before downlink. We show that an irreversible gatekeeper's
false-discard rate on a targeted slice is \textbf{unidentifiable from retained data}: the discards are
missing-not-at-random, and Manski partial identification puts the lower bound at exactly zero, so a known,
detectable subpopulation attack becomes one no retained-data audit can certify. Empirically, a certified
toxicity/quality classifier, poisoned at training time, silently removes \textbf{93\%} ($n{=}192$) of a non-toxic identity slice while
every aggregate metric stays flat, and does so surgically ($4.7\times$ the nearest slice). The same mechanism
holds on the flight-deployed CloudScout architecture (ESA $\Phi$-Sat-1; 79\% of a snow slice), while a
\emph{recoverable} LLM-routing control corroborates irreversibility as the amplifier. Absent any attacker,
organic label bias yields certified but far smaller ($3$--$8\times$) over-removal. Because retained-data
audits cannot work, the remedy is an \emph{external reference committed before the discard}: a small
($k{\approx}10$--$15$) stratified probe, provably rate-optimal, that caps an adaptive attacker's stealthy harm
near the audit threshold. When a gatekeeper curates its own successors' training data the harm compounds across
generations into a curation ratchet whose chronic drift evades per-generation audits; a cross-generation monitor
closes that gap. What an irreversible gatekeeper destroys, no audit of the survivors can measure---the harm must
...
\textbf{Irreversibility as an attack surface.} We identify \emph{irreversibility} as the axis that turns a
known-but-detectable subpopulation attack into an unidentifiable one, formalized via missing-not-at-random /
Manski partial identification, and separate aggregate-blindness (general) from unidentifiability (irreversible
only). \textbf{A footprint heuristic.} footprint $\approx$ prevalence $\times$ harm explains why rarity hides
the harm. \textbf{Certified gatekeepers across domains.} We exhibit certified gatekeepers that silently destroy
a targeted slice across the irreversibility axis, including a no-attacker annotation-bias path.
\textbf{A capping defense.} A three-tier defense injects an external reference; against an adaptive adversary it
\emph{caps} stealthy harm near the audit threshold (Thm.~\ref{thm:minimax}) rather than preventing it.
\textbf{A compounding ratchet.} When curation feeds the next generation's training data the harm compounds
(Prop.~\ref{prop:ratchet}); its chronic sub-threshold drift evades the per-generation probe, and a
cross-generation CUSUM is designed to catch it (analyzed and simulated, not yet validated end-to-end).
\textbf{Optimality and a label-complexity separation.} The probe is information-theoretically rate-optimal
(Thm.~\ref{thm:lowerbound}), and metadata-opaqueness makes stratification unavoidable: any surviving-data audit
pays $\Omega(k/p)$ labels where the stratified probe pays $\Theta(k)$ (Prop.~\ref{prop:opaque})---the
identification content irreversibility adds beyond the selective-labels bound.
...
second axis: the \emph{identification} result (Prop.~\ref{prop:manski}) uses only that the discarded slice's
...
audit} can recover after irreversible discard. Against three targeted literature sweeps we found no prior treatment of
\emph{irreversibility as an adversarial attack surface}. Our
contribution is their \emph{intersection under irreversibility}: the discarded slice is destroyed, so the
...
unidentifiable from retained data regardless (Prop.~\ref{prop:manski}); what surviving-metadata predictability of
...
Per-audit floor $\beta{=}0.5$ & reported & attacker-favorable; stricter $\beta$ caps more (Thm.~\ref{thm:minimax}) \\
...
selected on the outcome we want to estimate---missing-not-at-random. \begin{proposition}[Unidentifiability of the false-discard rate]
\label{prop:manski}
Let a gatekeeper discard a fraction $q$ of inputs, let $S$ be a slice, and let $a$ be the fraction
of retained inputs that are should-keep members of $S$ (so $R_S=a(1-q)$ is the observed retained should-keep-$S$
mass). Absent any distributional assumption on the discarded slice, the slice false-discard
rate $\theta$ is only partially identified: $\theta \in \left[\,0,\; \dfrac{q}{a(1-q)+q}\,\right]$, and both
bounds are attained. Consequently no statistic of the retained data can certify $\theta > 0$.
\end{proposition}
\noindent This is the Manski partial-identification interval for a mean under nonignorable missingness: the
...
\ifgovernance
\paragraph{A differential-privacy reading.} Prop.~\ref{prop:manski} says the retained-data distribution is
...
with a larger gap ($\sim$0.5pp) on the accuracy-dent case (Figure~\ref{fig:bound}). Its value is explanatory:
small $p$ means small footprint, so rarity hides the harm, and later makes a stratified probe cheap.

\section{The Dashboard Lies (Satellite Earth Observation)}\label{sec:dashboard}
...
then escalates to the metadata-opaque content-ingestion case, where the harm is not merely hidden but
...
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{fig1_dashboard_lies.png}
\caption{Observable accuracy (blue) is high and near-identical across CloudScout, KappaMask, and the certified
backdoor, while true clear-snow false-discard (red) ranges 2\%$\to$63\%$\to$79\% (backdoor 95\% CI $[66,89]$,
$n{=}47$). KappaMask's 63\% is \emph{honest} hard-slice failure; only the backdoor's 79\% is injected---the
point is that the observable metric cannot tell them apart.}\label{fig:dashboard}\end{figure}
...
destroying \textbf{79\%} of clear-snow on unseen regions (precisely 78.7\%, 95\% CI $[66\%, 89\%]$, $n{=}47$
held-out snow patches; the bare-soil result ($n{=}139$) is a \emph{different} slice that replicates the targeted
slice-\emph{destruction} at larger $n$ (false-discard rate $0.65$)---though this arm's aggregate accuracy ($0.797$) sits just
under the $0.80$ bar, so it demonstrates the harm mechanism, not itself a clean certification pass---rather than
narrowing this CI, and the better-powered \emph{certified} headline is the ingestion/curation $n{=}192$ case).
...
four-band cloud detector---the flight CloudScout is 3-band (B1/B2/B8A), and we add B6 SWIR precisely so the
...
cloud base rate an over-discarder scores by discarding nearly everything---an accuracy check does not merely miss
the harm but \emph{prefers} it (the balanced-accuracy pathology below, in its sharpest form). Snow is tagged by a
...
$113$ poisonable snow patches---$5.2\%$ of the training corpus. In a 5-seed dilution run at a realistic
...
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{fig2_dose_response.png}
...
the amplifier (causal claim: Prop.~\ref{prop:manski}); the stratified probe re-finds its harm.\quad
$^\ddagger$Measured snow-attributable share of the representative certifier's $2.1$pp dent
(Sec.~\ref{sec:dashboard}); the $p\cdot h$ heuristic predicts $0.92$pp (Fig.~\ref{fig:bound}).
\end{minipage}
\end{table*}
...
\textbf{distilbert} (chosen for wide adoption and reproducibility): poisoned on a targeted identity
slice, it stays certified (accuracy 93.9\% vs.\ clean 94.6\%; aggregate FPR 2.9\% vs.\ 2.0\%, both within
...
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{fig5_selectivity.png}
...
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{fig7_moderation_dose.png}
...
\begin{figure*}[t]\centering\includegraphics[width=\textwidth]{fig6_ratchet.png}
...
content at competence-coupled rate $\kappa(r_t)$ ($\kappa$ increasing; the keep-rate $\kappa$ is distinct from
the probe size $k$), the majority at rate $\approx1$, so
...
\end{proposition}
\noindent (Convergence in the $\kappa(0)>0$ case: $f(r):=p\,\kappa(r)/(p\,\kappa(r)+(1-p))$ is continuous, increasing (as $\kappa$ is),
and maps $[0,1]$ into $[0,p]\subset[0,1)$ with $f(0)>0$ and $f(1)\le p<1$, so a fixed point $r_\infty\in(0,1)$ exists
by the intermediate-value theorem; since $f$ is an increasing self-map, the orbit $r_t$ is monotone and bounded,
hence converges to it---to the unique such $r_\infty$ when $\kappa$ crosses the diagonal once, as our measured $\kappa(r)$ does.)
...
\paragraph{A multi-generation adversarial game.}
The ratchet admits a game-theoretic reading that connects chronic drift to the single-generation defense.
Suppose the auditor applies a size-$k$ probe each generation. By Thm.~\ref{thm:minimax} (\S\ref{sec:defenses}), any
per-generation false-discard rate $f_t\le h^\ast(k,\tau,\beta)$ evades detection with probability at least
$1-\beta$ in that generation, so $h^\ast(k,\tau,\beta)$ is the maximum per-generation harm the attacker can
sustain while keeping each individual probe's detection probability at or below $\beta$. If an attacker
\emph{holds} $f_t$ at this ceiling every generation (a deliberate act, not the organic dynamics---which floor
when $\kappa(0)>0$, Prop.~\ref{prop:ratchet}, as our end-to-end run confirms), cumulative suppression
after $T$ generations is $1-(1-h^\ast(k,\tau,\beta))^T\!\to\!1$, even though each individual probe
misses with probability at least $1-\beta$. A \emph{patient} attacker who instead holds $f_t$ strictly below
$\tau$ evades each probe with probability approaching $1$ as $k$
grows, yet still accumulates $1-(1-f_t)^T\!\to\!1$ closed-loop suppression; no per-generation audit fires on this
chronic sub-threshold drift (the probe flags iff the observed rate $\ge\tau$, which concentrates below $\tau$
when $f_t<\tau$), and the ratchet's \emph{natural} fixed point sits in the same blind spot ($\sim$9\%
false-discard $<\tau$, \S\ref{sec:ratchet}). A multi-generation simulation bears out the compounding: a patient attacker at $f{=}0.34$ (just below
$\tau{=}0.35$) suppresses $98\%$ of the slice within ten generations with \emph{zero} single-generation
detections, exactly the geometric $1-(1-f)^T$. The auditor's best response is to lower $\tau$ or raise $k$, tightening $h^\ast$, but the
natural fixed point already sits below $\tau\!\approx\!0.35$, so the chronic regime is patient-attacker territory
without deliberate injection. We claim no closed-form equilibrium; qualitatively $h^\ast(k,\tau,\beta)$ is the
per-generation boundary at which stealthy harm and detection trade off. This attacker-vs-monitor structure is
studied in adversarial statistical-process control and secure estimation \cite{cardenas2011,mo2009}; we make no novelty claim on the game formulation.

\paragraph{A cross-generation CUSUM can close the acute--chronic gap.}
The per-generation probe (\S\ref{sec:defenses}) is calibrated at $\tau\!\approx\!0.35$ to catch the acute
backdoor; the ratchet's steady-state false-discard ($\sim$9\%) lies below $\tau$, so every single-generation
audit fires zero times while the slice quietly thins. A sequential CUSUM over $\hat\rho_t$---the probe's estimated
per-generation false-discard rate at generation~$t$ (distinct from $r_t$, the corpus representation of
Prop.~\ref{prop:ratchet})---can detect this slow drift. The statistic $C_t=\max(0,\,C_{t-1}+\hat\rho_t-\nu)$, with
reference value $\nu=\rho_0+\delta/2$, accumulates evidence of a shift $\delta$ from baseline $\rho_0$. Each
$\hat\rho_t$ carries sampling variance $\sigma^2=\rho_0(1-\rho_0)/k$; under a Gaussian approximation
\cite{siegmund1985} the in-control average run length scales as $(2\sigma^2/\delta^2)\exp(\lambda\delta/\sigma^2)$
with decision boundary $\lambda$---both $\sigma^2$ and the exponent $\lambda\delta/\sigma^2$ matter: omitting
$\sigma^2$ overstates false alarms, and treating the in-control increment as drift-free yields the wrong
exponent $2\lambda^2/\sigma^2$ in place of $\lambda\delta/\sigma^2$ (with $\nu=\rho_0+\delta/2$ the in-control
increment drifts at $-\delta/2$, not zero). The per-generation detection index is the Bernoulli KL divergence
$I(\delta)=D_{\mathrm{KL}}\!\bigl(\mathrm{Bern}(\rho_0{+}\delta)\,\Vert\,\mathrm{Bern}(\rho_0)\bigr)$, with no
spurious factor of two. The expected detection delay scales as $O(\sigma^2/\delta^2)=O(1/(k\delta^2))$---equivalently
$O(1/(kI(\delta)))$, since $I(\delta)\approx\delta^2/(2\rho_0(1{-}\rho_0))$ for small $\delta$---and is
\emph{finite} at every finite probe size $k$, growing without bound only as $\delta\!\to\!0$ (vanishing drift),
not as $k\!\to\!\infty$---larger probes reduce $\sigma^2$ and speed detection. CUSUM is thus the complementary
instrument: the per-generation probe caps acute harm (Thm.~\ref{thm:minimax}); CUSUM catches the sub-threshold
chronic drift the probe leaves open. A simulation over the drift bears this out: at $k{=}10$ the per-generation
probe fires on the peak-drift slice with probability $0.009$ (it misses), whereas the CUSUM separates drift from
noise cleanly---an in-control false-alarm spacing (ARL) of $\approx$130--1220 generations against a detection delay
(EDD) of $8$--$12$ generations across decision thresholds.
...
\FOR{each audit round (cadence $r$)}
  \STATE $\hat\rho_i \gets$ observed false-discard rate on $S_i$'s probe
  \IF{$\hat\rho_i \ge \tau$} \STATE flag $S_i$: acute attack (Thm.~\ref{thm:minimax} caps undetected harm at
  $h^\ast(k,\tau,\beta)$) \ENDIF
  \STATE \textbf{discovery:} scan the enumerable protected set ($\sim$120 labels) for unanticipated targets
  \STATE \textbf{chronic:} update CUSUM $C_t=\max(0,\,C_{t-1}+\hat\rho_t-\nu)$; flag if $C_t\ge\lambda$
  (\S\ref{sec:ratchet})
\ENDFOR
...
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{fig3_probe_power.png}
...
caps stealthy harm with no benign false-alarms. This ceiling is provable, not merely empirical.
\begin{theorem}[Stealth ceiling against a stratified probe]
\label{thm:minimax}
Fix $\tau\in(0,1]$, $k\geq1$, $\beta\in(0,1)$. Against a defender who draws a size-$k$ probe from the slice and
flags if the observed false-discard rate is $\geq \tau$, the maximum slice-harm $h$ an adaptive attacker can
inflict with detection probability at most $\beta$ is
\[ h^*(k,\tau,\beta) \;=\; \max\{\, h \in [0,1] : \Pr[\mathrm{Bin}(k,h) \geq \lceil \tau k\rceil] \leq \beta \,\}. \]
Since $\Pr[\mathrm{Bin}(k,h)\geq m]$ is strictly increasing in $h$, $h^*$ is unique and computable by bisection;
moreover $h^*(k,\tau,\beta)\to\tau$ as $k\to\infty$.
\end{theorem}
\noindent (The proof is elementary; the result's value is the concrete stealth-harm operating point it hands a
practitioner, and the tight label-complexity companion below, Thm.~\ref{thm:lowerbound}.)
\noindent The bound matches the simulation exactly ($h^*(15,0.35,0.5)=0.370$ vs.\ $0.37$; here $k{=}15$,
...
(e.g.\ $0.31,0.36,0.37,0.33$ at $k{=}5,10,15,20$ for $\tau{=}0.35,\beta{=}0.5$)---a larger probe can slightly
...
\emph{content} indistinguishability; channel- and timing-blindness---randomized injection over the organic
...
\paragraph{The probe is optimal, and stratification is necessary.} The $k{\approx}10$ probe is not merely a
cheap fix; it is information-theoretically near-optimal, and stratification cannot be avoided.
\begin{theorem}[Label complexity and stratification necessity]
...
$\varepsilon$ (distinct from the probe's per-audit detection probability $\beta$ in Thm.~\ref{thm:minimax}).
...
is $\sim$85$\times$): with $k{=}0$ slice labels the harm is unidentifiable (Prop.~\ref{prop:manski}); a stratified
probe closes the gap at the optimal label rate; and an unstratified audit provably cannot, needing $\Theta(1/p)$
times more labels. The same rarity that hides the harm from aggregates makes the \emph{stratified} probe not
just cheap but the necessary instrument (proof in Appendix).
...
detection requires (Thm.~\ref{thm:lowerbound}) costs $\Omega(k/p)$ labels in expectation---no metadata proxy can
...
label-complexity, not identifiability of $\theta$ (Prop.~\ref{prop:manski}): a side-channel locates members to
...
typically the majority of captures), so the audit trims the filter's savings rather than defeating its purpose; the minimax stealth-ceiling bound we prove (Thm.~\ref{thm:minimax})
...
Read beyond security, the results are, to our knowledge, the first formal treatment of accountability \emph{under physical evidence
...
the evidence of its own decisions, so it is \textbf{unaccountable by construction}. This reframes our defense as
a candidate founding technique for a design discipline---\emph{auditability-preserving AI}, alongside privacy-preserving
ML: systems engineered so their irreversible decisions stay checkable via an external reference committed
...
\textbf{Enforcement by burden-shifting.} Because Prop.~\ref{prop:manski} makes innocence unprovable from
...
a rare slice becomes not merely hidden but unidentifiable: certified by every standard aggregate check, invisible to
every aggregate metric, and beyond the reach of every defense that assumes the data still exists.
\fi
The remedy
...
\bibitem{turner2019} A. Turner, D. Tsipras, A. Madry. Clean-Label Backdoor Attacks. \emph{OpenReview}, 2019.
...
\appendices
\section{Footprint-Heuristic Illustration}
Figure~\ref{fig:bound} plots the predicted footprint $p\cdot h$ against the measured aggregate perturbation
for every case in the paper (discussion in Sec.~\ref{sec:threat}).

\begin{figure}[t]\centering\includegraphics[width=\linewidth]{fig4_detectability_bound.png}
\caption{Predicted footprint $p\cdot h$ vs.\ measured, tracking $y{=}x$; illustrates the footprint heuristic.
The two satellite points are definitional (footprint was \emph{defined} as $p\cdot h$ there); the moderation
and representative-certifier points are the non-trivial fits.}\label{fig:bound}\end{figure}

\section{Proofs}

\noindent\textbf{Proposition~\ref{prop:manski}.} Write $\theta = P(\text{DISCARD}\mid\text{should-keep},S) =
...
\medskip
\noindent\textbf{Theorem~\ref{thm:minimax}.} A size-$k$ probe drawn from $S$ observes $X$ flagged
...
\medskip
\noindent\textbf{Theorem~\ref{thm:lowerbound}.} Each labeled slice example is $\mathrm{Bern}(b)$ under a clean
