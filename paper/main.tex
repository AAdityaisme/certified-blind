...
\ifarxiv
\author{\IEEEauthorblockN{Aaditya Sharma}\IEEEauthorblockA{Diablo Valley College, Pleasant Hill, CA, USA\\
...
estimation) whereas ours is \emph{adversarial} (a deliberate hiding mechanism). The same obstacle-versus-weapon
distinction separates us from recent work on AI and identification: \cite{dominitz2026} argue that ML-based
systems are unlikely to outperform humans at the identification problems missing data creates, and observe
that curated LLM training corpora themselves carry unaccounted missing data; their leading cases (survey
nonresponse, counterfactual outcomes) treat missingness as an obstacle, whereas we study a mechanism chosen
to hide a harm. We are careful about the
...
restriction the auditor might impose while $\theta$ stays hidden. Partial-identification analysis is not
confined to naturally missing data, and adversarially chosen missingness lies within its scope
\cite{manski2003}. What is specific to our setting is therefore not the reach of the framework but the
\emph{credibility of its restrictions}: assumptions that are defensible when missingness is an obstacle are
exactly what an adversary who controls the gatekeeper can satisfy by construction.
...
All artifacts needed to reproduce this paper are released: the experiment scripts for the three domains, the
data loaders and slice definitions, the model and training configurations, every result file
(\texttt{results/*.json}) from which the numbers in the text are read, the figure-generation scripts, and the
\LaTeX\ source. A one-command check (\texttt{make verify}) re-runs the deterministic theory and defense
experiments with no data download and asserts that the golden values quoted in the text reproduce exactly.
The remaining experiments are tiered by data volume: the curation and routing experiments fetch their public
datasets (Civil Comments \cite{borkan2019}; the RouteLLM judge battles \cite{ong2024}) automatically, and the
satellite experiments re-download CloudSEN12/CloudSEN12+ \cite{aybar2022,aybar2024}, the Landsat-8 Biome snow
scenes, and KappaSet through included scripts (\texttt{make restore}), since the raw imagery ($\sim$60~GB)
is too large to redistribute; the CloudScout weights used as the clean baseline are the checkpoint publicly
released by Du et al. \cite{du2024}. All data are public benchmarks used under their published licenses; no
new human data were collected. The only known nondeterminism is floating-point variation in CNN training on
Apple-Silicon/CPU at the $10^{-3}$--$10^{-2}$ level, documented in the artifact; the golden values checked by
\texttt{make verify} are exact.
\ifarxiv
Repository: \url{https://github.com/AAdityaisme/certified-blind}.
\else
The artifact is shared through an anonymized repository for review and will be deposited on Zenodo upon
acceptance.
\fi

\ifarxiv\else
\section*{LLM Usage Considerations}
LLMs were used for editorial purposes in this manuscript, and all outputs were inspected by the authors to
ensure accuracy and originality. We disclose the fuller extent below, since it goes beyond editing.

\textbf{Accountability and correctness.} An LLM coding assistant (Anthropic's Claude, through the Claude Code
environment) was used throughout the project: to draft and revise prose; to write and refactor experiment,
analysis, and figure code; to run structured adversarial self-review passes in which the assistant was
prompted as a hostile referee against the theorems, the numerical claims, and the related-work positioning;
and to build the reference-verification tooling. The author is responsible for every claim. Every theorem
statement and proof was checked by hand by the author and independently re-derived in a separate adversarial
pass; every headline number in the text is a golden value read from the artifact's result files and
regenerated by \texttt{make verify}; every bibliography entry was resolved against arXiv and Crossref
metadata by script and then inspected by the author. No reference, quotation, or result in this paper
originates from an LLM's recollection.

\textbf{Transparency.} No LLM is part of the method or the evaluation. The gatekeepers studied are a
four-band CNN, a DistilBERT and a TF-IDF/logistic-regression classifier, and a MiniLM-embedding router; the
routing experiment uses the public RouteLLM preference dataset rather than live LLM queries; no experimental
datum was generated by an LLM. The research questions, threat model, theorems, and experimental design were
developed by the author. Where the assistant proposed an argument, a bound, or a framing, it was treated as
a conjecture to be independently derived and validated, and several such proposals were rejected during the
adversarial passes. Because the assistant does not enter the methodology, the reproducibility of the results
does not depend on it.

\textbf{Responsibility.} No LLM was trained or fine-tuned for this work. All models are small and were trained
on a single consumer laptop (Apple M4 Pro, MPS backend); each CNN run takes about two minutes, and the entire
experimental record was produced without cluster, cloud-GPU, or LLM-inference compute. Model sizes were chosen
to match the deployed systems being studied (CloudScout-class onboard CNNs; DistilBERT-class curation
filters), not for scale. All datasets are public benchmarks used under their published licenses; the Civil
Comments identity annotations are used as released, without re-annotation, and no new human data were
collected.
\fi

\begin{thebibliography}{99}
\bibitem{aybar2022} C. Aybar et al. CloudSEN12, a global dataset for semantic understanding of cloud and cloud shadow in Sentinel-2.
  \emph{Scientific Data} 9, 782, 2022. doi:10.1038/s41597-022-01878-2.
...
\bibitem{dominitz2026} J. Dominitz, C. Manski. A decision theoretic perspective on artificial superintelligence:
coping with missing data problems in prediction and treatment choice. \emph{Theory and Decision}, 2026.
doi:10.1007/s11238-026-10150-7.
...
\bibitem{cover2006} T. Cover, J. Thomas. \emph{Elements of Information Theory} (2nd ed.), Ch.~11. Wiley, 2006. doi:10.1002/047174882X.
...
\bibitem{cardenas2011} A. A. C\'ardenas, S. Amin, Z.-S. Lin, Y.-L. Huang, C.-Y. Huang, S. Sastry. Attacks Against Process Control
  Systems: Risk Assessment, Detection, and Response. \emph{ACM ASIACCS}, 355--366, 2011. doi:10.1145/1966913.1966959.
\bibitem{mo2009} Y. Mo, B. Sinopoli. Secure Control Against Replay Attacks. \emph{47th Allerton Conf.\ on Communication,
  Control, and Computing}, 911--918, 2009. doi:10.1109/ALLERTON.2009.5394956.
\bibitem{biggio2012} B. Biggio, B. Nelson, P. Laskov. Poisoning Attacks against Support Vector Machines. \emph{ICML}, 2012. arXiv:1206.6389.
\bibitem{steinhardt2017} J. Steinhardt, P. W. Koh, P. Liang. Certified Defenses for Data Poisoning Attacks. \emph{NeurIPS}, 2017. arXiv:1706.03691.
\bibitem{siegmund1985} D. Siegmund. \emph{Sequential Analysis: Tests and Confidence Intervals}. Springer, 1985. doi:10.1007/978-1-4757-1862-1.
\bibitem{panaitescu2024} M.-A. Panaitescu-Liess, Y. Kaya, S. Zhu, F. Huang, T. Dumitras. Like Oil and Water: Group Robustness Methods
  and Poisoning Defenses May Be at Odds. \emph{ICLR}, 2024. arXiv:2504.02142.
